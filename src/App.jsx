  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bgGradient, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "2rem", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ marginBottom: "2.5rem", textAlign: "center", animation: "fadeIn 0.3s ease-out" }}>
        ...
        <div style={{ fontFamily: "Inter, sans-serif", fontSize: "0.75rem", fontWeight: 500, color: T.textDim, letterSpacing: "0.12em", textTransform: "uppercase" }}>Commercial Invoice Converter</div>
      </div>
      ...
      >
        ...
        {phase === "done" && isBatch && (
          <BatchDoneState results={results} successCount={successCount}
...
function ProcessingState({ progress }) {
  ...
  return (
    <div style={{ textAlign: "center", padding: "1rem 0" }}>
      ...
      {progress.total > 1 && (
        <>
          <p style={{ fontSize: "0.75rem", color: T.textDim, marginBottom: "0.5rem" }}>Bestand {progress.i} van {progress.total}</p>
...
function PreviewTable({ rows, totalItems }) {
  ...
  return (
    <div style={{ marginTop: "1.25rem", borderRadius: 10, overflow: "hidden", border: `1px solid ${T.line}` }}>
      <div style={{ padding: "0.4rem 0.75rem", background: T.panelDeep, borderBottom: `1px solid ${T.line}`, fontSize: "0.7rem", color: T.textDim, fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase" }}>
        Preview eerste {rows.length} regels
      </div>
      ...
      {totalItems > rows.length && (
        <div style={{ padding: "0.35rem 0.75rem", background: T.panelDeep, borderTop: `1px solid ${T.line}`, fontSize: "0.7rem", color: T.textGhost }}>
          + {totalItems - rows.length} meer regels in het Excel-bestand
        </div>
      )}
    </div>
  );
...
function ValidationBadge({ qty, total }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ fontSize: "0.72rem", color: T.good }}>Validatie geslaagd</span>
    </div>
  );
}

function PartialWarning({ result, onForceDownload }) {
  return (
    <div style={{
      background: "#1a1200", border: `1px solid ${T.warn ?? "#b86e00"}`, borderRadius: 10,
      padding: "0.85rem 1rem", marginBottom: "1rem",
    }}>
      <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.5rem" }}>
        <AlertTriangle size={16} color="#f59e0b" />
        <span style={{ fontSize: "0.82rem", fontWeight: 600, color: "#f59e0b" }}>Aantal regels komt niet overeen</span>
      </div>
      <p style={{ fontSize: "0.78rem", color: T.textDim, marginBottom: "0.75rem", lineHeight: 1.5 }}>
        Gevonden: <strong style={{ color: T.text }}>{result.qty} regels · CHF {fmtCHF(result.total)}</strong><br />
        Verwacht:  <strong style={{ color: T.text }}>{result.expectedQty} regels · CHF {fmtCHF(result.expectedTotal)}</strong>
      </p>
...
function SingleDoneState({ result, onReset, onRedownload, onForceDownload }) {
  ...
  return (
    <div style={{ padding: "0.5rem 0" }}>
      <div style={{ textAlign: "center", marginBottom: "1.25rem" }}>
        ...
        {isOk && (
          <>
            ...
            <button type="button" onClick={onRedownload} style={{
              ...
              <FileDown size={13} /> Opnieuw downloaden
            </button>
          </>
        )}
        ...
        {isPartial && (
          <>
            <p style={{ fontWeight: 600, color: T.text, marginBottom: "0.75rem" }}>Controleer de resultaten</p>
          </>
        )}
      </div>

      {isPartial && <PartialWarning result={result} onForceDownload={onForceDownload} />}

      {isOk && result.preview?.length > 0 && (
        <PreviewTable rows={result.preview} totalItems={result.qty} />
      )}
...
function BatchDoneState({ results, successCount, errorCount, partialCount, onDownloadZip, onDownloadSingle, onForceDownload, onReset }) {
      ...
      <div style={{ textAlign: "center", marginBottom: "1.5rem" }}>
        ...
        {(errorCount > 0 || partialCount > 0) && (
          <p style={{ fontSize: "0.8rem", color: T.textDim, marginTop: "0.25rem" }}>
            ...
            {errorCount > 0 && partialCount > 0 && " · "}
            {partialCount > 0 && `${partialCount} met waarschuwing`}
          </p>
        )}
      </div>
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                ...
                {r.isPartial && (
                  <p style={{ fontSize: "0.7rem", color: "#f59e0b", marginTop: "0.2rem", marginLeft: 17 }}>
                    Gevonden {r.qty} van {r.expectedQty} regels
                  </p>
                )}
