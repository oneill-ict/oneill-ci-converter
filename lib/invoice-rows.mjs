...
function inferColumns(lines, grid, names) {
  const itemRows = lines.filter(looksLikeItemRow);
  const sample = (i) => itemRows
    .map(l => l.runs.find(r => Math.abs(r.x - grid[i].x) <= X_TOLERANCE))
    .filter(Boolean).map(r => r.text);

  const qtyIdx   = names.indexOf("quantity");
  const discIdx  = names.indexOf("discount");
  const tariffIx = names.indexOf("tariffNo");

  // Price: an unnamed column between quantity and discount whose cells are money
  // and which reproduces the line total as quantity x price - discount. If the
  // arithmetic does not hold on most rows it is not the price and stays unnamed.
  if (qtyIdx >= 0 && discIdx > qtyIdx + 1) {
    for (let i = qtyIdx + 1; i < discIdx; i++) {
      ...
      if (totIdx < 0) continue;
      let checked = 0, agreed = 0;
      for (const line of itemRows) {
        const at = (j) => line.runs.find(r => Math.abs(r.x - grid[j].x) <= X_TOLERANCE)?.text;
        const q = parseAmount(at(qtyIdx)), p = parseAmount(at(i));
        const d = parseAmount(at(discIdx)) ?? 0, t = parseAmount(at(totIdx));
        if (q == null || p == null || t == null) continue;
        checked++;
        if (Math.abs(Math.round((q * p - d) * 100) / 100 - t) <= 0.02) agreed++;
      }
      if (checked >= 3 && agreed / checked >= 0.9) { names[i] = "price"; break; }
    }
  }
...
export function readItemRows(lines) {
  ...
  names = inferColumns(lines, grid, names);
  ...
  for (const line of lines) {
    ...
    rows.push({
      ...
      discount:    parseAmount(cells.discount) ?? 0,
      total,
      page:        line.page,
    });
  }
  ...
  return { rows, columns, skipped, unplaced, currency };
}
