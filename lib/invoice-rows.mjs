...
// Two runs are on the same visual line when their baselines differ by less than this
// share of their text height. Relative, not absolute: one invoice renders its whole
// header block at text height 0.349 instead of 7.5, so its rows sit 0.7 points apart
// where the item table's are 13.5, and a fixed 2-point tolerance merged four header rows
// into one. A fixed bucket was also arbitrary in a second way — Math.round(y / 2) put
// two runs 1.4 apart in the same bucket or in neighbouring ones depending on where they
// happened to fall.
//
// Verified against the fixed rule across all 45 corpus invoices: identical item rows,
// quantities, totals, skips and prices on every one.
  ...
  await pdfParse(pdfBuffer, {
    ...
    pagerender: async (pageData) => {
      ...
      for (const it of tc.items) {
        ...
        // Height drives the line tolerance, so it has to be a real number. pdf.js
        // reports it, transform[3] is the vertical scale behind it, and 1 keeps a
        // degenerate item from collapsing every line into one.
        const h = Number(it.height) || Math.abs(d) || 1;
        runs.push({ page, x, y, h, text });
      }
 ...
 *
 * Separate from extractLines, and exported, because this is where the judgement is: the
 * PDF reading around it is a thin adapter, but a wrong line here silently reshapes every
 * field downstream. It also could not be tested while it was buried inside a function
 * that needs a PDF — the committed fixtures store the *output* of this step, so a change
 * to it was invisible to them. test-extract-lines.mjs feeds it raw runs instead.
 *
 * Each run needs { page, x, y, h, text }.
 */
export function groupIntoLines(runs) {
  // Walk down the page and start a new line when the baseline drops further than the
  // text can account for.
  const sorted = [...runs].sort((a, b) => a.page - b.page || b.y - a.y || a.x - b.x);
  const lines = [];
  let current = null;
  for (const r of sorted) {
    const tol = Math.max(r.h, current ? current.maxHeight : 0) * LINE_SHARE;
    if (!current || r.page !== current.page || Math.abs(current.y - r.y) > tol) {
      current = { page: r.page, y: r.y, maxHeight: r.h, runs: [r] };
      lines.push(current);
    } else {
      current.runs.push(r);
      current.maxHeight = Math.max(current.maxHeight, r.h);
    }
  }
...
export function readItemRows(lines) {
  ...
  for (const line of lines) {
    ...
    // A quantity of pieces is a whole number: all 2,469 rows in the corpus are, so a
    // fraction means a cell was misread rather than that someone shipped 601.04 items.
    // Cheap and decisive — a glued quantity-and-price cell reads as 601.04 and is
    // refused here instead of travelling on to be caught by the footer. If an invoice
    // ever does bill by length or weight, this refuses it and names the line, which is
    // the right way to be wrong about it.
    const wholeQty = quantity == null || Number.isInteger(quantity);

    if (!/^\d{10}$/.test(tariffNo) || quantity == null || total == null || !wholeQty) {
      skipped.push({
        page: line.page,
        ...
        // A stable key, not a sentence. These cross into the frontend, which translates
        // them per locale — and Dutch strings here meant the lookup never matched and the
        // English UI showed Dutch. `detail` carries what a key cannot.
        reason: !/^\d{10}$/.test(tariffNo) ? "no tariff number"
              : quantity == null ? "no quantity"
              : !wholeQty ? "fractional quantity"
              : "no line total",
        detail: !wholeQty && quantity != null ? String(quantity) : null,
